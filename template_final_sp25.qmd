...
count <- 0
while(sum < 30){
  rand_no <- sample(1:10, 1, replace = TRUE)
  print(rand_no)
  sum <- sum + rand_no
  count <- count + 1
}
print(paste("You generated", count, "random numbers that sum to", sum))
```
...
```{r}
vec_matrix <- cbind(vec1, vec2, vec3)
vec_matrix
```
...
```{r}
max_col <- apply(vec_matrix, 2, max)
max_col
```
...
```{r}
even_sum <- function(data) {
  return ((sum(data))%%2 == 0)
}
sapply(vec_list, even_sum)
```
...
```{r}
fruit_info[max(fruit_info$weight), ]
```
...
for (i in nrow(fruit_info)) {
  if (fruit_info[i, 3] > 100) {
    fruit_info[i, 4] <- "heavy"
  } else{
    fruit_info[i, 4] <- "light"
  }
}
fruit_info
```
